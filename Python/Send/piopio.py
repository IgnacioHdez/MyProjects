    ...
    while ctSearches<Nsearch:
        for i in range(Ntopics):
            ...
            if ctIDReached[i]==0:
                ...
                if maxid[i]==0:
                    twit=api.GetSearch(term=topics[i],count=100,result_type="recent",lang='es')
                else:
                    twit=api.GetSearch(term=topics[i],count=100,result_type="recent",max_id=maxid[i]-1,lang='es')
                ...
                if ctSearches>=Nsearch:
                    break
            #Check if every search  has reached its limit
            if ctIDReached==np.ones([Ntopics]).tolist():
                break
        if ctIDReached==np.ones([Ntopics]).tolist():
            break
    #----------Save the results----------
...
print('Starting Program at   ' + dt.datetime.today().isoformat())
print('---------------------------------')
print()
T=dt.datetime.today()
tm.sleep(15*60)
while True:
    if abs((dt.datetime.today()-T).total_seconds()) > 15*60 :
