...
//1:(M) Show matrix A(i,j) trough the console
void plotm(int i, int j, double **A);

//2:(M) Returns the product of A(af,ac)*B(bf,bc)
double** prodm(double **A,int af,int ac,double **B,int bf,int bc);

//3:(M) Creates a matrix full of elements of value n
double** numbersm(int fa, int ca, double n);

//4:(M) Ask for a matrix trough console
double** elementm(int af,int ac);

//5:(F) Derivate of a function around x0
double dervf(double (*f)(double),double x0,double dx);

//6:(V) Returns the scalar product
double prodescv(int n, double *v, double *w);

//7:(V) Calculate the module (norm) of a vector
double normv(int n, double *v);

//8:(V) Show vector v(i,j) trough the console
...
//35:(V) Calcula la integral de dos vectores uno de x y otro de y
double* integralxy(int n, double *vx, double *vy);

//36:(V) Toma dos vectores y devuelve la derivada de y(x) para cada x
double* dervxy(int n, double *vx, double *vy);

//37:(V) Crea un vector cuyos elementos van de x0 a xf en n pasos
double* linspacev(double x0, double xf, int n);

//38:(F) Devuelve la solucion de la edo de primer orden cuya derivada es dydx(x,y)
double* edof(double (*dydx)(double,double),double* x,int n,double y0);

//39:(V) Crea un vector que une los vectores r1 y r2
double* r12v(int n,double *r1,double *r2);

//40:(I) Devuelve el minimo de dos enteros
int minimum(int i, int j);

//41:(I) Devuelve el mayor de dos enteros
int maximum(int i, int j);

//42:(F) Devuelve el factorial
...
double* edof(double (*dydx)(double,double),double* x,int n,double y0)
{double h, k1, k2,k3,k4;;double* y=numbersv(n,0);
y[0]=y0;
for(int i=1; i<n; i++)
{h=(x[i]-x[i-1]);
k1=dydx(x[i-1],y[i-1]);k2=dydx(x[i-1]+0.5*h, y[i-1]+0.5*k1*h);
k3=dydx(x[i-1]+0.5*h, y[i-1]+0.5*k2*h);k4=dydx(x[i-1]+h,y[i-1]+k3*h);
y[i]=y[i-1]+(1.0/6.0)*(k1+2*k2+2*k3+k4)*h;}
return y;}
...
{if (n<15)
    ...
    {double result=pow(n,n)*exp(-n)*sqrt(2*pi*n);
