...
//18:(F) Resolucion de un sistem de ecuaciones lineales mediante Gauss-Seidel
double** gssystem(double** A,int n , double** b,double err);

//19:(F) Crea un vector cuyos elementos son todos n
double* numbersv(int d, double n);

//20:(F) Graba un vector en el archivo namev
void writev(int n, double *v, const char* namev);

//21:(F) Lee un vector del archivo namev
...
void writem(int n, int m, double **A, const char* namemat)
{
 ...
 cout<<"Grabando matriz en el archivo "<<namemat<<" ..."<<endl;
 for (int i=0; i<n; i++)
 {for (int j=0; j<m; j++)
 {
 sal<<A[i][j]<<" ";
...
cout<<"Leyendo matriz del archivo "<<namemat<<" ..."<<endl;
 for (int i=0; i<n; i++)
 {for (int j=0; j<m; j++)
    ent>>A[i][j];}
 ent.close();
...
{
 ofstream sal(namev);
 cout<<"Grabando vector en el archivo "<<namev<<" ..."<<endl;
 for (int i=0; i<n; i++)
 {
 sal<<v[i]<<endl;
 }
...
cout<<"Leyendo vector del archivo "<<namev<<" ..."<<endl;
 for (int i=0; i<n; i++)
    {ent>>v[i];}
 ent.close();
 cout<<"Lectura completada "<<endl;
