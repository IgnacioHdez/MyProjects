...
double derparxyz(double (*F)(double,double,double),double x0,double y0,double z0,double dx,int xyz)
{
    if (xyz==1)return (-F(x0-dx,y0,z0)+F(x0+dx,y0,z0))/(2*dx);
    if (xyz==2)return (-F(x0,y0-dx,z0)+F(x0,y0+dx,z0))/(2*dx);
    if (xyz==3)return (-F(x0,y0,z0-dx)+F(x0,y0,z0+dx))/(2*dx);
}
//--------------------------------------------------------------------------------------------
//24:
double** idm(int n)
...
{m=A[i][i];
 for(int k=0;k<n ;k++)
  {A[i][k]=A[i][k]/m;I[i][k]=I[i][k]/m;}
 for(int h=i+1; h<n; h++)
  {m=A[h][i];
   for(int k=0; k<n; k++)
    {A[h][k]=A[h][k]-A[i][k]*m;I[h][k]=I[h][k]-I[i][k]*m;}}}
m=A[n-1][n-1];
for (int i=0; i<n; i++)
{A[n-1][i]=A[n-1][i]/m;I[n-1][i]=I[n-1][i]/m;}
for (int i=n-1; i>=0; i--)
{for(int h=i-1; h>=0; h--)
 {m=A[h][i];
  for(int k=n-1; k>=0; k--)
