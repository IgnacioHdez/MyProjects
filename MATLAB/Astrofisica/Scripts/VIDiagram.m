...
disp('Loading data for VIDiagram')
load('../Data/result.mat')

disp('Creating figure VIDiagram.fig')
%Create Figure
fig3=figure('Color',[1,1,1]);

%Set axes
axes1 = axes('Parent',fig3,'YMinorGrid','on','XMinorGrid','on');
box(axes1,'on');
grid(axes1,'on');
hold(axes1,'on');
...
% Create line of Sun
annotation(fig3,'line',[0.13125 0.9046875],...
    [0.666666666666667 0.666666666666667],'Color',[1 0 1],'LineStyle','--');

% Create line of Sun
annotation(fig3,'line',[0.4609375 0.458333333333333],...
...
for i=1:N
    ...
    %Plot Stars
    plot(VI(i),Mabs(i),'Marker','o','MarkerFaceColor',c(i,:),'MarkerEdgeColor',(c(i,:)+0.3*[1,1,1])/norm(c(i,:)+0.3*[1,1,1]),'MarkerSize',7)
    hold on
    ...
    if i>1 && ISpType(i,2)~='A' && ISpType(i,2)~='D' && ISpType(i,2)~=last(1) && ISpType(i,2)~=last(2) && ISpType(i,2)~=last(3) && ISpType(i,2)~=last(4) && ISpType(i,2)~=last(5)
        last(h1)=ISpType(i,2);
        h1=h1+1;
        text(BV(i),Mabs(i),['  ',ISpType(i,2)],'Color',[0,0,0],'FontWeight','bold')
    end
...
for i=1:max(size(rare))
    text(VI(rare(i)),Mabs(rare(i)),ISpType(rare(i),:),'Color',[0,0,0])
end
for i=1:N
    if HIP(i)== 3829
    text(VI(i),Mabs(i),'   White Dwarf 3829','Color',[0,0,0])
    end
end

disp('Saving VIDiagram figure')

savefig('../Figures/VIDiagram.fig')
